...
del i, base_URL, folder, im, images_dict, pix_val_1, total
gc.collect()
...
del folder, i, im, img_dict, pix_val_1, prefix_file
gc.collect()
...
BASE_URL = '../Datasets/Stanford Cars/'
dir_contents = os.listdir(BASE_URL)

img_list = {}
for direc in dir_contents:
    img_list[direc] = os.listdir(BASE_URL + direc)

del dir_contents, direc
gc.collect()

count = 0
pixel_list = []
for folder in img_list:
    for img in img_list[folder]:
        count += 1
        print(str(BASE_URL) + str(folder) + '/' + str(img) + '\t\t\t' +str(count))
        URL = BASE_URL + folder + '/' + img
        
        im = Image.open(URL).convert('L')
        im = im.resize((28,28))
        pix_val = list(im.getdata())
        pixel_list.append(pix_val)

del count, folder, im, img, img_list, pix_val
gc.collect()

df_images = pd.DataFrame(pixel_list)
df_images.to_csv('../Datasets/Non-text-images-Stanford.csv', index=False)

del pixel_list, df_images
gc.collect()

#---------------STL-10 Datset--------------------------------
BASE_URL = '../Datasets/STL Images/'
dir_contents = os.listdir(BASE_URL)
...
df_images = pd.DataFrame(pixel_list)
df_images.to_csv('../Datasets/Non-text-images-STL.csv', index=False)

del pixel_list, df_images
gc.collect()
#----------------Join all the datasets into 1.----------
img_101 = pd.read_csv('../Datasets/Non-text-images-101.csv')
img_256 = pd.read_csv('../Datasets/Non-text-images-256.csv')
img_stanford = pd.read_csv('../Datasets/Non-text-images-Stanford.csv')
img_stl = pd.read_csv('../Datasets/Non-text-images-STL.csv')

df_final = pd.concat([img_101, img_256, img_stanford, img_stl], axis=0)

del img_101, img_256, img_stanford, img_stl
gc.collect()

df_final.to_csv('../Datasets/Non-text-images-2.csv', index=False)
